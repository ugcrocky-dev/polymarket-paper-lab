...
export async function GET() {
  ...
  const top = state.bots
    .map((b) => {
      ...
      const outcomes = botTradeOutcomes(b);
      return {
        id: b.id,
        status: b.status,
  ...
  const deskOutcomes = sumTradeOutcomes(
    top.map((b) => ({
      positiveTrades: b.positiveTrades,
      negativeTrades: b.negativeTrades,
      flatTrades: 0,
  ...
  const families = familyIds.map((family) => {
    ...
    const outcomes = sumTradeOutcomes(
      bots.map((b) => ({
        positiveTrades: b.positiveTrades,
        negativeTrades: b.negativeTrades,
        flatTrades: 0,
  ...
  return NextResponse.json({
    ...
    totalUnrealized,
    totalTrades,
    positiveTrades: deskOutcomes.positiveTrades,
    negativeTrades: deskOutcomes.negativeTrades,
    profitPct: deskOutcomes.profitPct,
    top,
    fomo,
